...
A successful build *is* the proof check (type-checking a total, escape-hatch-free
definition is the proof). This verifies:
- Installation reversibility proof (`installReversible`)
- Idempotence proof (`doubleInstallIdempotent`)
- Echo-residue bridge (`Packages.Hello.Echo`)
- FFI type safety

=== Step 2: Generate C Headers from Idris2

```bash
idris2 --codegen c --build src/abi/oblibeny-abi.ipkg
# Generates the C ABI headers Zig links against
```
