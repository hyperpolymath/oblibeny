...
*Status:* 100% Complete *Date:* 2026-02-07
...
Public keys stored at: `+~/.obli-pkg/keyring/+` - `+dilithium5.pub+`
(2,592 bytes) - `+sphincsplus.pub+` (64 bytes) - `+ed25519.pub+` (32
bytes)

==== Package Database

Location: `+/var/lib/obli-pkg/installed.db+` Format: Tab-separated
values
...
Steps: 1. Verify triple signatures 2. Extract tar.gz archive 3. Check
dependencies 4. Install files to `+/usr/local/obli-pkg/+` 5. Register in
package database
...
The crypto FFI is called by the package manager during: 1. Package
installation (`+obli-pkg install+`) 2. Package verification
(`+obli-pkg verify+`) 3. Dependency resolution (future)
...
* *Post-quantum resistant*: Dilithium5 and SPHINCS+ protect against
quantum attacks
* *Triple signature*: Requires all three schemes to pass (defense in
depth)
* *No private key handling*: Only verification, not signing (separation
of concerns)
* *Keyring isolation*: Public keys in user home directory
* *Reversible operations*: Package database tracks installations for
rollback
...
. Optional: Prove formal properties in Idris2
. Optional: Add package signing tool (separate from obli-pkg)
. Optional: Implement dependency resolver
. Optional: Add rollback/uninstall with accountability traces
...
*Status:* ✅ CRYPTO FFI 100% COMPLETE *Completion:* 90% → 100%
(2026-02-07) *Estimated:* 2-3 hours *Actual:* 2.5 hours
