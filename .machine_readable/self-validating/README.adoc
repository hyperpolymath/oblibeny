...
This directory contains example K9 contractiles for common repository tasks, adapted to oblibeny (OCaml/dune build, Idris2 proof layer, Zig FFI):
...
Configuration with Nickel contracts for runtime validation. Evaluated safely without I/O. The example mirrors oblibeny's real CI gate (dune build, dune runtest, Idris2 proof type-check).
...
**STATE.a2ml**:: K9 components can generate or validate `.machine_readable/descriptiles/STATE.a2ml`
...
The repo-specific validator is `methodology-guard.k9.ncl`, pointed at
oblibeny's real proof layer (`src/abi/`) and metadata
