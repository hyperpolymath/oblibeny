...
let parse_args () =
  ...
  let specs = [
    ("--dump-ast", Arg.Unit (fun () -> opts.dump_ast <- true),
     " Dump parsed AST (OCaml show format)");
    ("--dump-sexpr", Arg.Unit (fun () -> opts.dump_sexpr <- true),
     " Dump parsed AST as S-expression");
    ("--dump-json", Arg.Unit (fun () -> opts.dump_json <- true),
...
let main () =
  ...
  if opts.dump_sexpr then begin
    prerr_endline "=== AST (S-expression) ===";
    print_endline (Oblibeny.Sexpr_dump.program_to_sexpr program)
  end;

  if opts.dump_json then begin
    prerr_endline "=== AST (JSON) ===";
    print_endline (Oblibeny.Sexpr_dump.program_to_json program)
  end;
